...
import { Check, Download, Minus, Plus, RotateCcw, X } from 'lucide-react';

// The poster preview grew zoom, pan, and a save action, and the pictures section wants
// exactly the same behaviour. Both now open this.
...
type ImageLightboxProps = {
  ...
  label: string;
  onClose: () => void;
  /** Shown under the image when a set is open, e.g. "3 / 12". */
...
export default function ImageLightbox({ src, label, onClose, positionLabel }: ImageLightboxProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const suggestedName = toSafeFileName(label, src);
  ...
  return createPortal(
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={`${label} fullscreen preview`}
      onClick={onClose}
    >
      ...
      <p className="pointer-events-none absolute bottom-3 left-1/2 -translate-x-1/2 border border-amberline/35 bg-black/60 px-2 py-1 font-mono text-[10px] uppercase tracking-[0.08em] text-cream/84">
        {positionLabel ? `${positionLabel} · ` : ''}Zoom {Math.round(zoom * 100)}% · Mouse wheel, buttons, drag to pan
      </p>
    </div>,
    document.body,
  );
